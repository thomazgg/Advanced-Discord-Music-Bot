        ...
        if(newState.id === client.user.id && oldState.serverDeaf === true && newState.serverDeaf === false)
            {
                try{
                    var channel = newState.member.guild.channels.cache.find(
                      ...
                      channel.send("Não ative meu som!, mutei meu som dnv! Isso protege os dados para que você tenha uma experiência mais rápida e suave")
                      newState.setDeaf(true);
                }catch (error) {
                    try{
                        console.log("Não foi possível enviar info msg em um botchat")
                        var channel = newState.member.guild.channels.cache.find(
                            channel =>
                              channel.type === "text" &&
                              channel.permissionsFor(newState.member.guild.me).has("SEND_MESSAGES")
                      ...
                      console.log("Não foi possível enviar mensagem de informações em um bate-papo aleatório")
                      newState.setDeaf(true);
                    }
                }
        }
